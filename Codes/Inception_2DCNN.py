...
class Inception:
    ...
    def Inception_v3(self):
        ...
        x = Conv_2D_Block(x, 80, 1, padding='valid')
        ...
        branch5x5 = Conv_2D_Block(x, 48, 1)
        branch5x5 = Conv_2D_Block(branch5x5, 64, 5)
        ...
        branch3x3dbl = Conv_2D_Block(x, 64, 1)
        branch3x3dbl = Conv_2D_Block(branch3x3dbl, 96, 3)
        branch3x3dbl = Conv_2D_Block(branch3x3dbl, 96, 3, strides=(2, 2), padding='valid')
        ...
        branch7x7dbl = Conv_2D_Block(x, 128, 1)
        branch7x7dbl = Conv_2D_Block(branch7x7dbl, 128, (7, 1))
        branch7x7dbl = Conv_2D_Block(branch7x7dbl, 128, (1, 7))
        branch7x7dbl = Conv_2D_Block(branch7x7dbl, 128, (7, 1))
        branch7x7dbl = Conv_2D_Block(branch7x7dbl, 192, (1, 7))

        branch_pool = tf.keras.layers.AveragePooling2D(pool_size=(3, 3), strides=(1, 1), padding='same')(x)
        branch_pool = Conv_2D_Block(branch_pool, 192, 1)
        x = tf.keras.layers.concatenate([branch1x1, branch7x7, branch7x7dbl, branch_pool], axis=-1, name='Inception_B1')

        for i in range(2, 4):
            branch1x1 = Conv_2D_Block(x, 192, 1)

            branch7x7 = Conv_2D_Block(x, 160, 1)
            branch7x7 = Conv_2D_Block(branch7x7, 160, (1, 7))
            branch7x7 = Conv_2D_Block(branch7x7, 192, (7, 1))

            branch7x7dbl = Conv_2D_Block(x, 160, 1)
            branch7x7dbl = Conv_2D_Block(branch7x7dbl, 160, (7, 1))
            branch7x7dbl = Conv_2D_Block(branch7x7dbl, 160, (1, 7))
            branch7x7dbl = Conv_2D_Block(branch7x7dbl, 160, (7, 1))
            branch7x7dbl = Conv_2D_Block(branch7x7dbl, 192, (1, 7))
        ...
        branch7x7 = Conv_2D_Block(x, 192, 1)
        branch7x7 = Conv_2D_Block(branch7x7, 192, (1, 7))
        branch7x7 = Conv_2D_Block(branch7x7, 192, (7, 1))

        branch7x7dbl = Conv_2D_Block(x, 192, 1)
        branch7x7dbl = Conv_2D_Block(branch7x7dbl, 192, (7, 1))
        branch7x7dbl = Conv_2D_Block(branch7x7dbl, 192, (1, 7))
        branch7x7dbl = Conv_2D_Block(branch7x7dbl, 192, (7, 1))
        branch7x7dbl = Conv_2D_Block(branch7x7dbl, 192, (1, 7))
        ...
        branch3x3 = Conv_2D_Block(x, 192, 1)
        branch3x3 = Conv_2D_Block(branch3x3, 320, 3, strides=(2, 2), padding='valid')

        branch7x7x3 = Conv_2D_Block(x, 192, 1)
        branch7x7x3 = Conv_2D_Block(branch7x7x3, 192, (1, 7))
        branch7x7x3 = Conv_2D_Block(branch7x7x3, 192, (7, 1))
        branch7x7x3 = Conv_2D_Block(branch7x7x3, 192, 3, strides=(2, 2), padding='valid')
        ...
        for i in range(2):
            branch1x1 = Conv_2D_Block(x, 320, 1)

            branch3x3 = Conv_2D_Block(x, 384, 1)
            branch3x3 = Conv_2D_Block(branch3x3, 384, (1, 3))
            branch3x3 = Conv_2D_Block(branch3x3, 384, (3, 1))

            branch3x3dbl = Conv_2D_Block(x, 448, 1)
            branch3x3dbl = Conv_2D_Block(branch3x3dbl, 384, 3)
            branch3x3dbl = Conv_2D_Block(branch3x3dbl, 384, (1, 3))
            branch3x3dbl = Conv_2D_Block(branch3x3dbl, 384, (3, 1))
    ...
    def Inception_v4(self):
        ...
        for i in range(4):
            ...
            branch5x5 = Conv_2D_Block(x, 64, 1)
            branch5x5 = Conv_2D_Block(branch5x5, 96, 3)
        ...
        for i in range(7):
            branch1x1 = Conv_2D_Block(x, 384, 1)

            branch7x7 = Conv_2D_Block(x, 192, 1)
            branch7x7 = Conv_2D_Block(branch7x7, 224, (1, 7))
            branch7x7 = Conv_2D_Block(branch7x7, 256, (7, 1))

            branch7x7dbl = Conv_2D_Block(x, 192, 1)
            branch7x7dbl = Conv_2D_Block(branch7x7dbl, 192, (7, 1))
            branch7x7dbl = Conv_2D_Block(branch7x7dbl, 224, (1, 7))
            branch7x7dbl = Conv_2D_Block(branch7x7dbl, 224, (7, 1))
            branch7x7dbl = Conv_2D_Block(branch7x7dbl, 256, (1, 7))
        ...
        # Reduction B: 8 x 8 x 1280
        branch3x3 = Conv_2D_Block(x, 192, 1)
        branch3x3 = Conv_2D_Block(branch3x3, 192, 3, strides=(2, 2), padding='valid')

        branch7x7x3 = Conv_2D_Block(x, 256, 1)
        branch7x7x3 = Conv_2D_Block(branch7x7x3, 256, (1, 7))
        branch7x7x3 = Conv_2D_Block(branch7x7x3, 320, (7, 1))
        branch7x7x3 = Conv_2D_Block(branch7x7x3, 320, 3, strides=(2, 2), padding='valid')
        ...
        for i in range(3):
            branch1x1 = Conv_2D_Block(x, 256, 1)

            branch3x3 = Conv_2D_Block(x, 384, 1)
            branch3x3_1 = Conv_2D_Block(branch3x3, 256, (1, 3))
            branch3x3_2 = Conv_2D_Block(branch3x3, 256, (3, 1))
            branch3x3 = tf.keras.layers.concatenate([branch3x3_1, branch3x3_2], axis=-1)

            branch3x3dbl = Conv_2D_Block(x, 384, 1)
            branch3x3dbl = Conv_2D_Block(branch3x3dbl, 448, (3, 1))
            branch3x3dbl = Conv_2D_Block(branch3x3dbl, 512, (1, 3))
            branch3x3dbl_1 = Conv_2D_Block(branch3x3dbl, 256, (3, 1))
            branch3x3dbl_2 = Conv_2D_Block(branch3x3dbl, 256, (1, 3))
            branch3x3dbl = tf.keras.layers.concatenate([branch3x3dbl_1, branch3x3dbl_2], axis=-1)

            branch_pool = tf.keras.layers.AveragePooling2D(pool_size=(3, 3), strides=(1, 1), padding='same')(x)
            branch_pool = Conv_2D_Block(branch_pool, 256, 1)
            x = tf.keras.layers.concatenate([branch1x1, branch3x3, branch3x3dbl, branch_pool], axis=-1, name='Inception_C' + str(i))
        ...
        if self.auxilliary_outputs:
            model = tf.keras.Model(inputs, outputs=[final_output, aux_output_0, aux_output_1], name='Inception_v4')
